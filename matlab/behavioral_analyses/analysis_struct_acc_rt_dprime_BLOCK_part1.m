...
matloadnames = struct_pt1_matload_all;

for block = 1:7
    
    outfname = sprintf('Struct_acc_rt_dprime_Part1_BLOCK_%d', block);
    fid=fopen([outfname '.txt'], 'w'); % open the file
    fprintf(fid, 'SUBJCOUNT\t SUBJECT\t BLOCK\t overall_acc\t overall_rt\t overall_dprime\n'); %Create header
    
    struct_acc_rt_dprime_ALL = [];
    subjcount = 0;
    
    for c = 1:length(struct_pt1_matload_all) %REMEMBER, induct paths have participants who've completed whole complete roster
        ...
        all = loaded.all_tog;
        subj = all(1, 1); 
        struct = all(all(:, 2) == block, :); %ensuring from correct block
        ...
        %Col3: Trial#
        %Col4: Object
        %Col5: Rot/2back appearance (1/0)
            %0 = (ROT) 2back
            %1 = (NOTROT) none
        %Col6: RotResp(1)/NotRotResp(2)/NoResp(0)
            %1 = RespNot
            %2 = RespRot
            %0 = NoResp
        ...
        for d = 1:size(struct, 1)
            rot_status = struct(d, 5); %1 = not rot; 0 = rot
            rot_resp = struct(d, 6);   %1 = not rot resp; 2 = rot resp; 0 = no resp
        ...
        total_rot = sum(struct(:, 5) == 0);
        total_notrot = sum(struct(:, 5) == 1);
        ...
        %overall accuracy
        overall_acc_avg = nanmean(struct(:, 7));
        
        %overall RT
        overall_rt_avg = nanmean(struct(:, 8));
        
        %ovrall d-prime
        d_prime = hit_z - fa_z;

        
        struct_acc_rt_dprime = [subjcount, subj, block, overall_acc_avg, overall_rt_avg, d_prime];
        struct_acc_rt_dprime_ALL = [struct_acc_rt_dprime_ALL; struct_acc_rt_dprime];
        
        cd('/Users/athula/Dropbox/Experiments/TesserScan/Behavioral Results')
        
        %% Overall output to text file
        fprintf(fid, '%d\t %d\t %d\t %d\t %d\t %d\n',...
            subjcount, subj, block, overall_acc_avg, overall_rt_avg, d_prime);
    end
    
    close all
    
    %also save out a .mat file of the same .txt file
    save(sprintf('Struct_acc_rt_dprime_Part1_BLOCK_%d', block),'struct_acc_rt_dprime_ALL');
