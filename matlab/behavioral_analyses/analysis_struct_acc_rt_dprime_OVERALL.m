...
for c = 1:size(datapaths_all, 1)
    ...
    name = subjname{c};
    struct_name = sprintf('%s_StructLearn_AllTog.mat', name);
    struct = load(struct_name); 
    struct = struct.all_tog;
    subj = all(1, 1); 
...
save(sprintf('Struct_acc_rt_dprime_OVERALL'),'struct_acc_rt_ALL');